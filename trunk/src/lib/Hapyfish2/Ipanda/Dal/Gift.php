...
class Hapyfish2_Ipanda_Dal_Gift
{
     ...
     * Single Instance
     *
     * @return Hapyfish2_Ipanda_Dal_Gift
     */
    ...
    {
        if (APP_STATUS_DEV == 4) {
    		return 'ipanda_user_gift_bag';
    	}

    	$id = floor($uid/DATABASE_NODE_NUM) % 10;
    	return 'ipanda_user_gift_bag_' . $id;
    }

    public function getTableNameWish($uid)
    {
        if (APP_STATUS_DEV == 4) {
    		return 'ipanda_user_gift_wish';
    	}
    	$id = floor($uid/DATABASE_NODE_NUM) % 10;
    	return 'ipanda_user_gift_wish_' . $id;
    }

    public function getTableNameFriendWish($uid)
    {
        if (APP_STATUS_DEV == 4) {
    		return 'ipanda_user_gift_friend_wish';
    	}
    	$id = floor($uid/DATABASE_NODE_NUM) % 10;
    	return 'ipanda_user_gift_friend_wish_' . $id;
    }


    /* basic db table
   	update ipanda_gift g , ipanda_building b set g.name=b.name,g.class_name=b.class_name where g.gid=b.id and g.type=2;
	update ipanda_gift g , ipanda_item i set g.name=i.name,g.class_name=i.class_name where g.gid=i.id and g.type=1;
    */
    //
    public function getBasicGiftList()
    {
        ...
        $rdb = $db['r'];
    	$sql = "SELECT * FROM ipanda_gift WHERE is_online=1 ORDER BY sort ";
