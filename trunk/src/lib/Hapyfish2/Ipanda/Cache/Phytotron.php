...
class Hapyfish2_Ipanda_Cache_Phytotron
{
    public static function getPhytotronList($uid, $savehighcache = false)
    {
    	$key = 'i:u:phytotronlist:';
    ...
    public static function addrentlog($uid,$phytotronInfo,$num)
    {
    	$date = date("Ymd",time());
    	$key = 'i:u:phytotronrentlog:';
    	$key = Hapyfish2_Ipanda_Cache_Memkey::getuserkey($key);
        $key = $key . $phytotronInfo['uid'].":".$phytotronInfo['id'].":".$date;
        $cache = Hapyfish2_Cache_Factory::getMC($phytotronInfo['uid']);
        $ids = $cache->get($key);
        $ids[$uid] = $num;
        $cache->set($key, $ids,86400);
        return true;
    }
