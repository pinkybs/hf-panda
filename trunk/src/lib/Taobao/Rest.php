...
class Taobao_Rest
{
    ...
    //
    public function jianghu_sendmsg($target, $body, $method)
    {
        try {
	        $data = $this->jianghu->msg_publish($target, $body, $method);
	        return $data;
        }
        catch (Exception $e) {
            info_log('jianghu_sendmsg:'.$e->getMessage(), 'api_call_failed'.date('Ymd'));
        }
    ...
    public function isFan($uid = null)
    {
        if ($uid == null) {
        	$uid = $this->user_id;
        }
    	try {
	        $data = $this->jianghu->fan_check(SHOP_OWNER_ID, $uid);
    		if ($data && is_array($data) && isset($data['result'])) {
            ...
            info_log('isFan:'.$e->getMessage(), 'api_call_failed'.date('Ymd'));
        }

        return false;
    }

    public function beFan()
    {
    	try {
	        $data = $this->jianghu->fan_follow(SHOP_OWNER_ID);
    		if ($data && is_array($data) && isset($data['follow_result'])) {
	            if ($data['follow_result'] == 'true') {
	            	return true;
	            }
	        }
        }
        catch (Exception $e) {
            info_log('beFan:'.$e->getMessage(), 'api_call_failed'.date('Ymd'));
        }
